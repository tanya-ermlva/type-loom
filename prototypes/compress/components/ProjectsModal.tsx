...
/**
 * Modal that lists every saved project with its thumbnail. Per-row
 * actions: Load, Rename, Duplicate, Delete. Loading replaces the
 * working state; the auto-persist store will then also keep that
 * loaded state as the live working copy.
 */
export function ProjectsModal({ open, onClose }: Props) {
  ...
  // Refresh list whenever modal opens (cheap — at most a few projects).
  useEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      <div
        className="bg-zinc-900 border border-zinc-800 rounded-lg max-w-[900px] max-h-[80vh] w-full overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-center px-5 py-3 border-b border-zinc-800">
          <h2 className="text-zinc-100 font-semibold text-sm">Projects</h2>
